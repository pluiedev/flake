...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  bitmaps = fetchzip {
    url = "https://github.com/ful1e5/BreezeX_Cursor/releases/download/v${finalAttrs.version}/bitmaps.zip";
    hash = "sha256-JncEE+G4tPgcLgabe/dD7DOVMLOWCI7e+3VdmXuZSo8=";
  };

  buildPhase = ''
    runHook preBuild
    ctgen configs/x.build.toml -p x11 -d "$bitmaps/BreezeX-Dark" -n "BreezeX-Dark" -c "Extended KDE Dark (v${finalAttrs.version})"
    ctgen configs/x.build.toml -p x11 -d "$bitmaps/BreezeX-Black" -n "BreezeX-Black" -c "Extended KDE Black (v${finalAttrs.version})"
